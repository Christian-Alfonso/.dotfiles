...
Copy-Item `
    ...
    -Destination $VSCodeSettingsPath `
    -Force

Copy-Item `
    -Path "$PSScriptRoot\..\.vscode\keybindings.json" `
    -Destination $VSCodeKeybindingsPath `
    -Force
...
if (!$NoVSCodeInsiders) {
    ...
    $VSCodeInsidersSettingsPath = "$env:APPDATA\Code - Insiders\User\settings.json"
    $VSCodeInsidersKeybindingsPath = "$env:APPDATA\Code - Insiders\User\keybindings.json"
    ...
    # If the VSCode Insiders settings or keybindings paths do not
    # exist yet, they probably need to be created
    if ((!(Test-Path $VSCodeInsidersSettingsPath)) -or (!(Test-Path $VSCodeInsidersKeybindingsPath))) {
        Write-Warning "At least one of the VSCode Insiders config paths is missing, opening VSCode Insiders once to create them"
    ...
    Copy-Item `
        ...
        -Destination $VSCodeInsidersSettingsPath
        -Force

    Copy-Item `
        -Path "$PSScriptRoot\..\.vscode\insiders\keybindings.json" `
        -Destination $VSCodeInsidersKeybindingsPath `
        -Force
...
# Needed for the next step here to install PowerToys
