...
# After installation is done and configurations are copied over,
# install all of the extensions that we want, so we do not have
...
if (!$NoVSCodeInsiders) {
    ...
    if ((!(Test-Path $VSCodeInsidersSettingsPath)) -or (!(Test-Path $VSCodeInsidersKeybindingsPath))) {
        ...
        # Open VSCode Insiders once initially to let it create those paths for us
        # before copying over the configuration files
        & $VSCodeInsiders
    }

    # Replace configurations for VSCode
    Copy-Item -Path "$PSScriptRoot\..\.vscode\insiders\settings.json" -Destination $VSCodeInsidersSettingsPath
    Copy-Item -Path "$PSScriptRoot\..\.vscode\insiders\keybindings.json" -Destination $VSCodeInsidersKeybindingsPath
