...
param(
    ...
    # Do not install the Insiders version of VSCode
    # as well. Both normal and Insiders versions are
    # installed to allow for having different accounts
    # or configurations signed in on each version
    [switch] $NoVSCodeInsiders,

    # Do not install extensions from the extension list
    # for both normal VSCode and Insiders (if applicable)
    [switch] $NoVSCodeExtensionsInstall,
...
# Install core tools (Oh-My-Posh) via DSC. For the list of what is included
# and what is intentionally left out (e.g. Git), see the file itself.
$PackagesConfig = "$PSScriptRoot\packages.dsc.yaml"
winget configure $PackagesConfig

#
# Windows Terminal (WT) Setup
#

$OhMyPoshConfig = Convert-Path "$PSScriptRoot\..\theme-v2.omp.json"
Copy-Item `
    -Path $OhMyPoshConfig `
    -Destination "$env:USERPROFILE\theme-v2.omp.json" `
    -Force

# Legacy code, keeping in case the paths to OMP are needed again. Finding OMP
# for execution was used for installing fonts, but they do not persist on
# the WindowsApps version at least. This was replaced by the manual font
# installation that occurs near the end of this setup script.
#
...
Copy-Item `
    -Path "$PSScriptRoot\..\.wt\settings.json" `
...
# Install VSCode with context menu integration. WinGet DSC does not support
# installer override arguments, so this explicit call is needed for the shell
# context menu entries. If VSCode is already installed, WinGet upgrades it.
...
# Get VSCode binary location for installing extensions - try Get-Command first
# in case the PATH was already updated, then fall back to known install locations
$VSCodeCandidates = @(
    "$env:LOCALAPPDATA\Programs\Microsoft VS Code\bin\code.cmd",
    "$env:ProgramFiles\Microsoft VS Code\bin\code.cmd"
)
$VSCode = (Get-Command code -ErrorAction SilentlyContinue)?.Source ??
($VSCodeCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1)
if (!$VSCode) {
    throw "Could not find VSCode after installation, checked PATH and: $($VSCodeCandidates -join ', ')"
}

if ($NoVSCodeExtensionsInstall) {
    Write-Host "Skipping VSCode extension installation"
}
else {
    # The list found in this file can be generated by running "code --list-extensions"
    $ExtensionsList = Get-Content "$PSScriptRoot\..\.vscode\extensions.txt"
...
Copy-Item `
    ...
    -Destination "$VSCodeUserDir\settings.json" `
    -Force

# Copy over VSCode keybindings
Copy-Item `
    -Path "$PSScriptRoot\..\.vscode\keybindings.json" `
    -Destination "$VSCodeUserDir\keybindings.json" `
    -Force

# Do the exact same procedure for VSCode Insiders that was
# done for VSCode, if it has not been disabled by the switch
if (!$NoVSCodeInsiders) {
    #
    # VSCode Insiders Setup
    #

    # Install VSCode Insiders with context menu integration. WinGet DSC does not
    # support installer override arguments, so this explicit call is needed for
    # the shell context menu entries. If VSCode Insiders is already installed,
    # WinGet upgrades it.
    winget install --id Microsoft.VisualStudioCode.Insiders --source winget --override '/SILENT /MERGETASKS="addcontextmenufiles,addcontextmenufolders,associatewithfiles,addtopath"'

    # Get VSCode Insiders binary location for installing extensions - try Get-Command
    # first in case the PATH was already updated, then fall back to known install locations
    $VSCodeInsidersCandidates = @(
        "$env:LOCALAPPDATA\Programs\Microsoft VS Code Insiders\bin\code-insiders.cmd",
        "$env:ProgramFiles\Microsoft VS Code Insiders\bin\code-insiders.cmd"
    )
    $VSCodeInsiders = (Get-Command code-insiders -ErrorAction SilentlyContinue)?.Source ??
    ($VSCodeInsidersCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1)
    if (!$VSCodeInsiders) {
        throw "Could not find VSCode Insiders after installation, checked PATH and: $($VSCodeInsidersCandidates -join ', ')"
    }

    if ($NoVSCodeExtensionsInstall) {
        Write-Host "Skipping VSCode Insiders extension installation"
    }
    else {
        # The list found in this file can be generated by running "code-insiders --list-extensions"
        $ExtensionsInsidersList = Get-Content "$PSScriptRoot\..\.vscode\insiders\extensions.txt"
    ...
    $VSCodeInsidersUserDir = "$env:APPDATA\Code - Insiders\User"
    New-Item -ItemType Directory -Force -Path $VSCodeInsidersUserDir | Out-Null

    # Copy over VSCode Insiders settings
    Copy-Item `
        -Path "$PSScriptRoot\..\.vscode\insiders\settings.json" `
        -Destination "$VSCodeInsidersUserDir\settings.json" `
        -Force

    # Copy over VSCode Insiders keybindings
    Copy-Item `
        -Path "$PSScriptRoot\..\.vscode\insiders\keybindings.json" `
        -Destination "$VSCodeInsidersUserDir\keybindings.json" `
        -Force
}
...
# Install Neovim with dependencies using its own script, which include
# installing VS 2022 Build Tools, Windows 11 SDK, and adding directories
# to the system PATH, since WinGet does not always do this automatically.
$NeovimInstallDepConfig = Convert-Path "$PSScriptRoot\..\.nvim\InstallWithDependencies.ps1"
